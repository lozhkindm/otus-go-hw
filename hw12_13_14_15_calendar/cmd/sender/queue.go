...
func NewQueue(ctx context.Context, config Config) (app.Queue, error) {
	...
	case queueTypeRabbitMQ:
		...
		if err := rabbit.DeclareQueue(ctx, config.RabbitMQ.Queue.Name); err != nil {
			return nil, err
		}
		if err := rabbit.BindQueue(ctx, config.RabbitMQ.Queue.Name, config.RabbitMQ.Exchange.Name); err != nil {
			return nil, err
		}
		queue = rabbit
	default:
